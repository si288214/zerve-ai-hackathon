...
def fetch_finbert_score(ticker: str, event_type: str,
    ...
    THRESHOLD = {"earnings": 0.15, "drug_trial": 0.15, "merger": 0.10, "fed": 0.02, "cpi": 0.02}
    if abs(h) < THRESHOLD.get(event_type, 0.15):
        print(f"    [llm]  |h|={abs(h):.3f} below threshold → NO PREDICTION")
        return None
