...
ALPHA_VANTAGE_KEY = os.getenv("ALPHA_VANTAGE_KEY", "os.getenv("ALPHA_VANTAGE_KEY", "")")
FINNHUB_KEY       = os.getenv("FINNHUB_KEY", "os.getenv("FINNHUB_KEY", "")")
...
def fetch_finnhub_news(ticker: str, event_date: str,
                       lookback_days: int = 7, event_type: str = "earnings") -> tuple[Optional[float], list[str]]:
    """
    ...
    try:
        ...
        # Combine headline + summary for richer FinBERT input
        headlines = []
        for a in data[:50]:  # fetch more to compensate for filtering
        ...
        EVENT_KEYWORDS = {
            "earnings": {'earnings','estimates','eps','revenue','guidance','forecast',
                'outlook','quarter','beat','miss','profit','results','expectation',
                'analyst','consensus','sales','income','margin','growth','above','below'},
            "fed": {'fed','federal reserve','fomc','rate','inflation','interest',
                'powell','cut','hike','hold','pause','pivot','hawkish','dovish',
                'monetary','cpi','pce','jobs','employment','gdp'},
            "cpi": {'inflation','cpi','pce','prices','consumer price','core',
                'fed','rate','hot','cool','above','below','estimates'},
            "drug_trial": {'fda','approval','approved','pdufa','trial','drug','therapy',
                'phase','clinical','efficacy','safety','reject','adcom','nda','bla'},
            "merger": {'merger','acquisition','deal','acquire','buyout','takeover',
                'shareholder','vote','close','regulatory','antitrust','approval'},
        }
